...
- There is no section-jump navigation (no sidebar, no hamburger menu) — this was deliberately removed. `Nav.astro` is just a small floating bar, fixed to the top-right corner (`fixed inset-x-0 top-0`, content pushed right via `justify-end`), holding `LanguageSwitcher` + `ThemeToggle` + a print button (`window.print()`) + a "download as Markdown" link (`data:` URI of the raw resume file). It takes no `sections` prop. Don't reintroduce a `sections`-driven nav without discussion.
