...
- `src/components/` — presentational only, no business logic. `ResumePage.astro` is the single composition point both routes render through, and it owns the desktop two-column layout: `Header.astro` (name/title/photo only) renders full-width above it, then a `md:grid md:grid-cols-[14rem_1fr] md:gap-10` wrapper holds `Nav.astro` (left column) and a right column with `ContactInfo.astro` (email/phone/address/links), the summary, and all sections. This keeps the sidebar positioned relative to the content column (inside `max-w-3xl`, starting at the same height as the contact links) rather than pinned to the raw viewport edge.
- `Nav.astro` renders TWO separate `<nav>` elements — a mobile top bar (`md:hidden`, hamburger + collapsible section-links panel, `position: sticky` to the viewport) and a desktop column (`hidden md:sticky md:self-start`, a normal grid item that starts in flow at the top of the two-column wrapper and then stickies while scrolling) — each with its own `LanguageSwitcher`/`ThemeToggle` instance, since a single shared DOM node can't be both a grid column and a sticky top bar at once. The desktop nav's own content order is deliberate: section links, then a `border-t` divider, then the language switcher + theme toggle directly below it (NOT pinned to the bottom via `justify-between`/full-height flex — that was the earlier, wrong version).
