...
- YAML frontmatter: `name`, `title`, optionally `lang` (defaults to `en`), `photo` (markdown image syntax, e.g. `"![Alt](./photo.jpg)"`), `contact` (list of `{label, value, type}`, `type` is `email`, `phone`, or `text` — `email`/`phone` become clickable `mailto:`/`tel:` links, `text` renders as plain value; any number of fields, in any order, e.g. two phone numbers), `links` (list of markdown links, e.g. `"[GitHub](https://github.com/you)"`). Each contact field and link renders as its own line in the header, formatted `Label: value`, with `email`/`phone` values as clickable links — so it stays readable and clickable in print too.
...
- Within a section, `### Title | Org | Dates` starts a timeline entry (used for Experience, Education, etc). The `Org` piece can optionally be a markdown link (e.g. `### Senior Engineer | [Acme Corp](https://acme.com) | 2022 - Present`) to link the company/school name — plain text also works. Add a location with `### Title | Org | Location | Dates` (e.g. `### Senior Engineer | [Acme Corp](https://acme.com) | Montréal, QC | 2022 - Present`) — omit it and the entry falls back to the 3-piece form. Any text between the section heading and the first `###` entry renders as an intro paragraph above the timeline. A section with no `###` entries just renders as plain markdown.
